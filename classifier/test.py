...
tf.app.flags.DEFINE_boolean( 'watch', False, 
	                           """ Path to model to test. """)
tf.app.flags.DEFINE_boolean( 'video', False, 
	                           """ Testing with video or images. """)
...
# # Import graph
# try:
# 	info_msg("Loading graph form {}".format(FLAGS.meta_path))
# 	saver = tf.train.import_meta_graph(FLAGS.meta_path)
# 	done_msg()
# except:
# 	err_msg("Could not import graph. Is your meta file: {} ?".format(FLAGS.meta_path))
...
# done_msg()
# #except:
# #	err_msg("Could not load model parameters. Is your model path: {} ?".format(FLAGS.model_path))

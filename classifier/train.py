...
def train():
	...
	# Inputs 
	x = tf.placeholder(tf.float32, shape=[None, cfg.IMG_HEIGHT, cfg.IMG_WIDTH, cfg.NUM_CHANNELS], name='x')
	y = tf.placeholder(tf.float32, shape=[None, cfg.NUM_CLS], name='y')
	y_cls = tf.argmax(y, axis=1)
	
	info_msg("Loading architecture {}...".format(FLAGS.net))
	if FLAGS.net == 'alexnet':
		model =  N.Alexnet(cfg.NUM_CLS, cfg.KEEP_PROB)
		out = model.load_net(x, training=True)
		model_dict = cfg.ALEXNET

...
class Alexnet():
	...
	def load_net(self, x, train):
		conv1 = L.conv('conv1', x, 11, 96, 4, train=train)
		norm1 = L.lrn(conv1, 2, 2e-05, 0.75)
		pool1 = L.maxpool('pool1', norm1, 3, 2, padding='VALID')

		conv2 = L.conv('conv2', pool1, 5, 256, 1, groups=2, train=train)
		norm2 = L.lrn(conv2, 2, 2e-05, 0.75)
		pool2 = L.maxpool('pool2', norm2, 3, 2, padding='VALID')

		conv3 = L.conv('conv3', pool2, 3, 384, 1, train=train)
		conv4 = L.conv('conv4', conv3, 3, 384, 1, groups=2, train=train)
		conv5 = L.conv('conv5', conv4, 3, 256, 1, groups=2, train=train)
		pool5 = L.maxpool('pool5', conv5, 3, 2, padding='VALID') 

		flat  = L.flatten(pool5)
		...
		return L.fc('fc8', drp7, self.num_classes, relu=False)
